...
// NowPaymentsCurrencies returns the list of available currencies from NowPayments.
// GET /api/v1/admin/payment/nowpayments/currencies
func (h *PaymentHandler) NowPaymentsCurrencies(c *gin.Context) {
	currencies, err := h.paymentService.NowPaymentsCurrencies(c.Request.Context())
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
