...
// CheckPaymentStatus manually queries the upstream provider for the order's
// payment status. Rate-limited to 1 request per minute per order.
// POST /api/v1/payment/orders/:id/check-status
func (h *PaymentHandler) CheckPaymentStatus(c *gin.Context) {
	...
	order, err := h.paymentService.CheckOrderStatusRateLimited(c.Request.Context(), orderID)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
