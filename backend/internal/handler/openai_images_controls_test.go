...
func TestResolveOpenAIImagesRoutingModel_UsesChannelMappedModel(t *testing.T) {
	parsed := &service.OpenAIImagesRequest{Model: "gpt-image-2"}

	got := resolveOpenAIImagesRoutingModel(parsed, service.ChannelMappingResult{
		MappedModel: "gpt-image-1",
	})
	require.Equal(t, "gpt-image-1", got)

	got = resolveOpenAIImagesRoutingModel(parsed, service.ChannelMappingResult{})
	require.Equal(t, "gpt-image-2", got)

	got = resolveOpenAIImagesRoutingModel(nil, service.ChannelMappingResult{})
	require.Empty(t, got)
}
