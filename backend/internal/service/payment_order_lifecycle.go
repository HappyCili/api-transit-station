...
const manualCheckCooldown = time.Minute

// CheckOrderStatusRateLimited manually queries the upstream provider for the
// order's payment status, subject to a 1-request-per-minute rate limit per order.
func (s *PaymentService) CheckOrderStatusRateLimited(ctx context.Context, orderID int64) (*dbent.PaymentOrder, error) {
	s.manualCheckMu.Lock()
	last, ok := s.manualCheckMap[orderID]
	now := time.Now()
	if ok && now.Sub(last) < manualCheckCooldown {
		s.manualCheckMu.Unlock()
		retryAfter := manualCheckCooldown - now.Sub(last)
		return nil, infraerrors.TooManyRequests("RATE_LIMITED",
			fmt.Sprintf("please wait %.0f seconds before checking again", retryAfter.Seconds()))
	}
	s.manualCheckMap[orderID] = now
	s.manualCheckMu.Unlock()
