...
func (s *PaymentService) CreateOrder(ctx context.Context, req CreateOrderRequest) (*CreateOrderResponse, error) {
	...
	if err != nil {
		slog.Error("get payment config failed", "user_id", req.UserID, "error", err)
		return nil, fmt.Errorf("get payment config: %w", err)
	}
	if !cfg.Enabled {
		slog.Warn("payment system disabled", "user_id", req.UserID)
		return nil, infraerrors.Forbidden("PAYMENT_DISABLED", "payment system is disabled")
	}
	plan, err := s.validateOrderInput(ctx, req, cfg)
	if err != nil {
		slog.Warn("order input validation failed", "user_id", req.UserID, "error", err)
		return nil, err
	}
	if err := s.checkCancelRateLimit(ctx, req.UserID, cfg); err != nil {
		slog.Warn("cancel rate limited", "user_id", req.UserID, "error", err)
		return nil, err
	}
	...
	if plan != nil {
		...
		slog.Debug("plan price applied", "user_id", req.UserID, "plan_price", plan.Price)
	} else if req.OrderType == payment.OrderTypeBalance {
		orderAmount = calculateCreditedBalance(req.Amount, cfg.BalanceRechargeMultiplier)
		slog.Debug("balance order credited amount", "user_id", req.UserID, "credited_amount", orderAmount)
	}
	...
	if s.configService != nil {
		...
		if err != nil {
			slog.Error("validate method currency failed", "user_id", req.UserID, "payment_type", req.PaymentType, "error", err)
			return nil, err
		}
	}
	slog.Debug("create order currency info",
		"user_id", req.UserID,
	...
	if err != nil {
		slog.Error("calculate pay amount failed",
			"user_id", req.UserID,
			"limit_amount", limitAmount,
			"fee_rate", feeRate,
	...
	slog.Info("create order calculated amount",
		"user_id", req.UserID,
		"pay_amount", payAmount,
		"pay_amount_str", payAmountStr,
		"currency", methodCurrency,
	)
	sel, err := s.selectCreateOrderInstance(ctx, req, cfg, payAmount)
	if err != nil {
		slog.Error("select instance failed", "user_id", req.UserID, "payment_type", req.PaymentType, "error", err)
		return nil, err
	}
	slog.Info("instance selected",
		"user_id", req.UserID,
		"provider_key", sel.ProviderKey,
		"instance_id", sel.InstanceID,
		"payment_mode", sel.PaymentMode,
	)
	if err := s.validateSelectedCreateOrderInstance(ctx, req, sel); err != nil {
		slog.Error("selected instance validation failed", "user_id", req.UserID, "error", err)
		return nil, err
	}
	selectedCurrency := payment.DefaultPaymentCurrency
	if sel != nil {
		selectedCurrency = paymentProviderConfigCurrency(sel.ProviderKey, sel.Config)
		slog.Debug("selected provider currency", "provider_key", sel.ProviderKey, "currency", selectedCurrency)
	}
	if selectedCurrency != methodCurrency {
		slog.Debug("recalculating pay amount with selected currency",
			"user_id", req.UserID,
			"old_currency", methodCurrency,
		...
		if err != nil {
			slog.Error("recalculate pay amount failed",
				"user_id", req.UserID,
				"limit_amount", limitAmount,
				"fee_rate", feeRate,
		...
		slog.Info("pay amount recalculated",
			"user_id", req.UserID,
			"currency", selectedCurrency,
			"pay_amount", payAmount,
		)
	...
	if err := validateSelectedCreateOrderAmountCurrency(payAmountStr, sel); err != nil {
		slog.Error("pay amount validation failed",
			"user_id", req.UserID,
	...
	if err != nil {
		slog.Error("create order in db failed", "user_id", req.UserID, "error", err)
		return nil, err
	}
	slog.Info("order created in db", "user_id", req.UserID, "order_id", order.ID)
	resp, err := s.invokeProvider(ctx, order, req, cfg, limitAmount, payAmountStr, payAmount, plan, sel)
	if err != nil {
		slog.Error("invoke payment provider failed",
			"user_id", req.UserID,
			"order_id", order.ID,
			"provider_key", sel.ProviderKey,
			"error", err,
...
func (s *PaymentService) selectCreateOrderInstance(ctx context.Context, req CreateOrderRequest, cfg *PaymentConfig, payAmount float64) (*payment.InstanceSelection, error) {
	...
	slog.Debug("selecting payment instance",
		"payment_type", req.PaymentType,
		"pay_amount", payAmount,
	...
	if err != nil {
		slog.Error("select instance failed",
			"payment_type", req.PaymentType,
			"pay_amount", payAmount,
			"strategy", cfg.LoadBalanceStrategy,
			"error", err,
	...
	if sel == nil {
		slog.Warn("no available instance",
			"payment_type", req.PaymentType,
			"pay_amount", payAmount,
		)
...
func (s *PaymentService) invokeProvider(ctx context.Context, order *dbent.PaymentOrder, req CreateOrderRequest, cfg *PaymentConfig, limitAmount float64, payAmountStr string, payAmount float64, plan *dbent.SubscriptionPlan, sel *payment.InstanceSelection) (*CreateOrderResponse, error) {
	slog.Info("invoke payment provider",
		"order_id", order.ID,
		"user_id", req.UserID,
		"provider_key", sel.ProviderKey,
		"instance_id", sel.InstanceID,
	...
	if err != nil {
		slog.Error("create payment provider failed", "provider", sel.ProviderKey, "instance", sel.InstanceID, "error", err)
	...
	slog.Debug("calling provider create payment",
		"order_id", order.ID,
		"provider_key", sel.ProviderKey,
	...
	if err != nil {
		slog.Error("provider create payment failed",
			"order_id", order.ID,
			"user_id", req.UserID,
			"provider_key", sel.ProviderKey,
...
func classifyCreatePaymentError(req CreateOrderRequest, providerKey string, err error) error {
	...
	slog.Error("create payment error",
...
func buildCreateOrderResponse(order *dbent.PaymentOrder, req CreateOrderRequest, payAmount float64, sel *payment.InstanceSelection, pr *payment.CreatePaymentResponse, resultType payment.CreatePaymentResultType) *CreateOrderResponse {
	var payCurrencyAmount float64
	if pr.PayAmount != "" {
		payCurrencyAmount, _ = strconv.ParseFloat(pr.PayAmount, 64)
	}
	return &CreateOrderResponse{
		OrderID:      order.ID,
		Amount:       order.Amount,
