...
func buildOpenAIImagesResponsesRequest(parsed *OpenAIImagesRequest, toolModel string) ([]byte, error) {
	...
	if effort := strings.TrimSpace(parsed.ReasoningEffort); effort != "" {
...
func (s *OpenAIGatewayService) writeOpenAIImagesStreamEvent(c *gin.Context, flusher http.Flusher, account *Account, eventName string, payload []byte) error {
	if c != nil && c.Request != nil {
		localized, err := s.localizeOpenAIImagesResponseJSON(c.Request.Context(), c, account, payload)
		if err != nil {
			return err
		}
		payload = localized
...
func (s *OpenAIGatewayService) tryWriteOpenAIImagesStreamEvent(
	...
	if err := s.writeOpenAIImagesStreamEvent(c, flusher, account, eventName, payload); err != nil {
...
func (s *OpenAIGatewayService) handleOpenAIImagesOAuthNonStreamingResponse(
	...
	responseBody, err = s.localizeOpenAIImagesResponseJSON(c.Request.Context(), c, account, responseBody)
	if err != nil {
		return OpenAIUsage{}, 0, nil, err
	}
...
func (s *OpenAIGatewayService) handleOpenAIImagesOAuthStreamingResponse(
	...
	processData := func(dataBytes []byte) {
		...
		case "response.image_generation_call.partial_image":
			...
			s.tryWriteOpenAIImagesStreamEvent(c, flusher, account, &clientDisconnected, &lastDownstreamWriteAt, eventName, payload)
		case "response.output_item.done":
			img, itemID, ok, extractErr := extractOpenAIImageFromResponsesOutputItemDone(dataBytes)
			if extractErr != nil {
				s.tryWriteOpenAIImagesStreamEvent(c, flusher, account, &clientDisconnected, &lastDownstreamWriteAt, "error", buildOpenAIImagesStreamErrorBody(extractErr.Error()))
		...
		case "response.completed":
			...
			if len(finalResults) == 0 {
				...
				s.tryWriteOpenAIImagesStreamEvent(c, flusher, account, &clientDisconnected, &lastDownstreamWriteAt, "error", buildOpenAIImagesStreamErrorBody(outputErr.Error()))
		...
		case "error", "response.failed":
			if upstreamErr := openAIImagesUpstreamErrorFromSSEPayload(dataBytes); upstreamErr != nil {
				...
				if !clientDisconnected && (!retryable || c.Writer.Size() != writerSizeBeforeResponse) {
					s.tryWriteOpenAIImagesStreamEvent(c, flusher, account, &clientDisconnected, &lastDownstreamWriteAt, "error", buildOpenAIImagesStreamErrorBodyFromUpstream(upstreamErr))
				}
				setOpsUpstreamError(c, upstreamErr.clientStatusCode(), upstreamErr.clientMessage(), "")
	...
	finalizePending := func() error {
		...
		s.tryWriteOpenAIImagesStreamEvent(c, flusher, account, &clientDisconnected, &lastDownstreamWriteAt, "error", buildOpenAIImagesStreamErrorBody(streamErr.Error()))
		return streamErr
	}
	...
	if streamInterval <= 0 && keepaliveInterval <= 0 {
		...
		for {
			...
			if err != nil {
				...
				s.tryWriteOpenAIImagesStreamEvent(c, flusher, account, &clientDisconnected, &lastDownstreamWriteAt, "error", buildOpenAIImagesStreamErrorBody(err.Error()))
	...
	for {
		...
		case ev, ok := <-events:
			...
			if ev.err != nil {
				...
				s.tryWriteOpenAIImagesStreamEvent(c, flusher, account, &clientDisconnected, &lastDownstreamWriteAt, "error", buildOpenAIImagesStreamErrorBody(ev.err.Error()))
		...
		case <-intervalCh:
			...
			s.tryWriteOpenAIImagesStreamEvent(c, flusher, account, &clientDisconnected, &lastDownstreamWriteAt, "error", buildOpenAIImagesStreamErrorBody(fmt.Sprintf("upstream image stream idle for %s", streamInterval)))
...
func (s *OpenAIGatewayService) forwardOpenAIImagesOAuth(
	...
	upstreamParsed, err := s.prepareOpenAIImagesParsedForUpstream(parsed)
	if err != nil {
		return nil, err
	}
	responsesBody, err := buildOpenAIImagesResponsesRequest(upstreamParsed, requestModel)
	if err != nil {
		return nil, err
	}
	upstreamReq, err := s.buildUpstreamRequest(upstreamCtx, c, account, responsesBody, token, true, s.ExtractSessionID(c, parsed.Body), false)
	if err != nil {
		return nil, err
	}
	...
	writerSizeBeforeResponse := c.Writer.Size()
	if parsed.Stream {
		usage, imageCount, imageOutputSizes, firstTokenMs, err = s.handleOpenAIImagesOAuthStreamingResponse(resp, c, startTime, account, parsed.ResponseFormat, openAIImagesStreamPrefix(parsed), requestModel)
		if err != nil {
			if imageCount > 0 {
