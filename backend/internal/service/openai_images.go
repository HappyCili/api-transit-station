...
const (
	...
	openAIImagesUploadsEndpoint     = "/v1/uploads"

	openAIImagesGenerationsURL = "https://api.openai.com/v1/images/generations"
	openAIImagesEditsURL       = "https://api.openai.com/v1/images/edits"
	openAIImagesUploadsURL     = "https://api.openai.com/v1/uploads"
...
func parseOpenAIImagesJSONRequest(body []byte, req *OpenAIImagesRequest) error {
	...
	if reasoningEffort, err := parseOpenAIImagesReasoningEffort(body); err != nil {
		return err
	} else {
	...
	if references := gjson.GetBytes(body, "reference_images"); references.Exists() {
		if !references.IsArray() {
			return fmt.Errorf("invalid reference_images field type")
		}
...
func parseOpenAIImagesMultipartRequest(body []byte, contentType string, req *OpenAIImagesRequest) error {
	...
	for {
		...
		case "reasoning_effort", "reasoning.effort":
			reasoningEffort, err := normalizeOpenAIImagesReasoningEffort(value, true)
			if err != nil {
				return err
			}
			req.ReasoningEffort = reasoningEffort
...
func parseOpenAIImagesReasoningEffort(body []byte) (string, error) {
	for _, path := range []string{"reasoning_effort", "reasoning.effort"} {
		...
		if result.Exists() {
			...
			return normalizeOpenAIImagesReasoningEffort(result.String(), true)
		}
	}
	return "", nil
}

func normalizeOpenAIImagesReasoningEffort(value string, required bool) (string, error) {
	normalized := strings.ToLower(strings.TrimSpace(value))
	if normalized == "" {
		if required {
			return "", fmt.Errorf("reasoning_effort is required when provided")
		}
		return "", nil
	}
	switch normalized {
...
func (s *OpenAIGatewayService) forwardOpenAIImagesAPIKey(
	...
	forwardBody, forwardContentType, err := s.rewriteOpenAIImagesForwardBody(upstreamCtx, c, account, token, body, parsed.ContentType, upstreamModel)
	if err != nil {
		return nil, err
	}
...
func (s *OpenAIGatewayService) rewriteOpenAIImagesForwardBody(ctx context.Context, c *gin.Context, account *Account, token string, body []byte, contentType string, model string) ([]byte, string, error) {
	rewritten, rewrittenContentType, err := rewriteOpenAIImagesModel(body, contentType, model)
	if err != nil {
		return nil, "", err
	}
	mediaType, _, parseErr := mime.ParseMediaType(rewrittenContentType)
	if parseErr == nil && strings.EqualFold(mediaType, "multipart/form-data") {
		return rewritten, rewrittenContentType, nil
	}
	rewritten, err = s.rewriteOpenAIImagesLocalInputURLs(ctx, c, account, token, rewritten)
	if err != nil {
		return nil, "", err
	}
	return rewritten, rewrittenContentType, nil
...
func rewriteOpenAIImagesModel(body []byte, contentType string, model string) ([]byte, string, error) {
	...
	rewritten = stripOpenAIImagesInternalJSONOptions(rewritten)
	return rewritten, contentType, nil
}

func stripOpenAIImagesInternalJSONOptions(body []byte) []byte {
...
func (s *OpenAIGatewayService) rewriteOpenAIImagesLocalInputURLs(ctx context.Context, c *gin.Context, account *Account, token string, body []byte) ([]byte, error) {
	...
	if refs := gjson.GetBytes(body, "reference_images"); refs.IsArray() {
		for index, item := range refs.Array() {
			upstreamURL, replaced, err := s.openAIImagesInputURLToUpstreamReferenceURL(ctx, c, account, token, item.String(), uploadCache)
			if err != nil {
				return nil, err
			}
			if replaced {
				next, setErr := sjson.SetBytes(out, fmt.Sprintf("reference_images.%d", index), upstreamURL)
				if setErr != nil {
					return nil, setErr
				}
				out = next
	...
	if images := gjson.GetBytes(body, "images"); images.IsArray() {
		for index, item := range images.Array() {
			upstreamURL, replaced, err := s.openAIImagesInputURLToUpstreamReferenceURL(ctx, c, account, token, item.Get("image_url").String(), uploadCache)
			if err != nil {
				return nil, err
			}
			if replaced {
				next, setErr := sjson.SetBytes(out, fmt.Sprintf("images.%d.image_url", index), upstreamURL)
				if setErr != nil {
					return nil, setErr
				}
				out = next
	...
	if maskImageURL := gjson.GetBytes(body, "mask.image_url"); maskImageURL.Exists() {
		upstreamURL, replaced, err := s.openAIImagesInputURLToUpstreamReferenceURL(ctx, c, account, token, maskImageURL.String(), uploadCache)
		if err != nil {
			return nil, err
		}
		if replaced {
			next, setErr := sjson.SetBytes(out, "mask.image_url", upstreamURL)
			if setErr != nil {
				return nil, setErr
			}
			out = next
...
type openAIImagesReferenceUpload struct {
	FileName    string
	ContentType string
	Data        []byte
}

func (s *OpenAIGatewayService) openAIImagesInputURLToUpstreamReferenceURL(ctx context.Context, c *gin.Context, account *Account, token string, rawURL string, uploadCache map[string]string) (string, bool, error) {
	rawURL = strings.TrimSpace(rawURL)
	if rawURL == "" {
		return rawURL, false, nil
	}
	if uploadCache != nil {
		if cachedURL, ok := uploadCache[rawURL]; ok {
			return cachedURL, true, nil
		}
	}
	upload, replaced, err := s.openAIImagesReferenceUploadFromInputURL(rawURL)
	if err != nil {
		return "", false, err
	}
	if !replaced {
	...
	upstreamURL, err := s.uploadOpenAIImagesReferenceToUpstream(ctx, c, account, token, upload)
	if err != nil {
		return "", false, err
	}
	if uploadCache != nil {
		uploadCache[rawURL] = upstreamURL
	}
	return upstreamURL, true, nil
}

func (s *OpenAIGatewayService) openAIImagesReferenceUploadFromInputURL(rawURL string) (openAIImagesReferenceUpload, bool, error) {
	rawURL = strings.TrimSpace(rawURL)
	fileName, ok := localImageFileNameFromPublicURL(rawURL)
	if !ok {
		return openAIImagesReferenceUpload{}, false, nil
	}
	data, err := os.ReadFile(filepath.Join(localimagestore.StorageDir(s.imageStorageDataDir()), fileName))
	if err != nil {
		return openAIImagesReferenceUpload{}, false, fmt.Errorf("read local reference image %s: %w", fileName, err)
	}
	contentType := localImageContentType(fileName, data)
	if !strings.HasPrefix(strings.ToLower(contentType), "image/") {
		return openAIImagesReferenceUpload{}, false, fmt.Errorf("local reference file %s is not an image", fileName)
	}
	return openAIImagesReferenceUpload{
		FileName:    fileName,
		ContentType: contentType,
		Data:        data,
	}, true, nil
...
func (s *OpenAIGatewayService) uploadOpenAIImagesReferenceToUpstream(ctx context.Context, c *gin.Context, account *Account, token string, upload openAIImagesReferenceUpload) (string, error) {
	...
	targetURL := openAIImagesUploadsURL
	if account != nil {
		if baseURL := account.GetOpenAIBaseURL(); baseURL != "" {
			...
			targetURL = buildOpenAIImagesURL(validatedURL, openAIImagesUploadsEndpoint)
		}
	}

	var buffer bytes.Buffer
	writer := multipart.NewWriter(&buffer)
	if err := writer.WriteField("purpose", "reference"); err != nil {
		return "", fmt.Errorf("write upstream upload purpose: %w", err)
	}
	partHeader := make(textproto.MIMEHeader)
	partHeader.Set("Content-Disposition", mime.FormatMediaType("form-data", map[string]string{
		"name":     "file",
		"filename": firstNonEmptyString(strings.TrimSpace(upload.FileName), "reference"+extensionForImageContentType(upload.ContentType)),
	}))
	partHeader.Set("Content-Type", firstNonEmptyString(strings.TrimSpace(upload.ContentType), "application/octet-stream"))
	part, err := writer.CreatePart(partHeader)
	if err != nil {
		return "", fmt.Errorf("create upstream upload file part: %w", err)
	}
	if _, err := part.Write(upload.Data); err != nil {
		return "", fmt.Errorf("write upstream upload file: %w", err)
	}
	if err := writer.Close(); err != nil {
		return "", fmt.Errorf("finalize upstream upload: %w", err)
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, targetURL, bytes.NewReader(buffer.Bytes()))
	if err != nil {
		return "", err
	}
	...
	resp, err := s.httpUpstream.Do(req, openAIImageAccountProxyURL(account), accountID, accountConcurrency)
	if err != nil {
		return "", fmt.Errorf("upstream reference upload failed: %w", err)
	}
	if resp == nil {
		return "", fmt.Errorf("upstream reference upload returned empty response")
	}
	defer func() { _ = resp.Body.Close() }()
	respBody, readErr := io.ReadAll(io.LimitReader(resp.Body, 2<<20))
	if readErr != nil {
		return "", fmt.Errorf("read upstream upload response: %w", readErr)
	}
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		message := sanitizeUpstreamErrorMessage(extractUpstreamErrorMessage(respBody))
		if message == "" {
			message = fmt.Sprintf("upstream reference upload failed: status %d", resp.StatusCode)
		}
		return "", fmt.Errorf("%s", message)
	}
	upstreamURL := strings.TrimSpace(gjson.GetBytes(respBody, "url").String())
	if upstreamURL == "" {
		return "", fmt.Errorf("upstream reference upload response missing url")
	}
...
func (s *OpenAIGatewayService) prepareOpenAIImagesParsedForUpstream(parsed *OpenAIImagesRequest) (*OpenAIImagesRequest, error) {
	if parsed == nil {
		return nil, nil
	}
	prepared := *parsed
	if len(parsed.InputImageURLs) > 0 {
		prepared.InputImageURLs = make([]string, 0, len(parsed.InputImageURLs))
		for _, imageURL := range parsed.InputImageURLs {
			dataURL, replaced, err := s.localImagePublicURLToDataURL(imageURL)
			if err != nil {
				return nil, err
			}
			if replaced {
				prepared.InputImageURLs = append(prepared.InputImageURLs, dataURL)
			} else {
				prepared.InputImageURLs = append(prepared.InputImageURLs, imageURL)
			}
		}
	}
	if strings.TrimSpace(parsed.MaskImageURL) != "" {
		dataURL, replaced, err := s.localImagePublicURLToDataURL(parsed.MaskImageURL)
		if err != nil {
			return nil, err
		}
		if replaced {
			prepared.MaskImageURL = dataURL
		}
	}
	return &prepared, nil
}

func (s *OpenAIGatewayService) localImagePublicURLToDataURL(rawURL string) (string, bool, error) {
	fileName, ok := localImageFileNameFromPublicURL(rawURL)
	if !ok {
		return rawURL, false, nil
	}
	data, err := os.ReadFile(filepath.Join(localimagestore.StorageDir(s.imageStorageDataDir()), fileName))
	if err != nil {
		return "", false, fmt.Errorf("read local image %s: %w", fileName, err)
	}
	contentType := localImageContentType(fileName, data)
	if !strings.HasPrefix(strings.ToLower(contentType), "image/") {
		return "", false, fmt.Errorf("local file %s is not an image", fileName)
	}
	return fmt.Sprintf("data:%s;base64,%s", contentType, base64.StdEncoding.EncodeToString(data)), true, nil
}

func localImageFileNameFromPublicURL(rawURL string) (string, bool) {
	rawURL = strings.TrimSpace(rawURL)
	if rawURL == "" {
		return "", false
	}
	...
	escapedName := strings.TrimPrefix(pathValue, prefix)
...
func localImageContentType(fileName string, data []byte) string {
...
func (s *OpenAIGatewayService) handleOpenAIImagesNonStreamingResponse(resp *http.Response, c *gin.Context, account *Account) (OpenAIUsage, int, []string, error) {
	...
	body, err = s.localizeOpenAIImagesResponseJSON(c.Request.Context(), c, account, body)
	if err != nil {
		return OpenAIUsage{}, 0, nil, err
	}
...
func (s *OpenAIGatewayService) handleOpenAIImagesStreamingResponse(
	...
	processLine := func(line []byte) error {
		...
		localized, err := s.localizeOpenAIImagesStreamLine(c.Request.Context(), c, account, line)
		if err != nil {
			return err
		}
		line = localized
	...
	for {
		...
		case <-intervalCh:
			...
			_ = s.writeOpenAIImagesStreamEvent(c, flusher, account, "error", buildOpenAIImagesStreamErrorBody(fmt.Sprintf("upstream image stream idle for %s", streamInterval)))
...
func newOpenAIImageHTTPStatusError(resp *http.Response, fallback string) error {
	if resp == nil {
		if strings.TrimSpace(fallback) == "" {
			fallback = "openai image backend request failed"
		}
...
func (s *OpenAIGatewayService) imageStorageDataDir() string {
...
func (s *OpenAIGatewayService) imagePublicBaseURL() string {
	if s != nil && s.cfg != nil {
...
func (s *OpenAIGatewayService) localizeOpenAIImagesResponseJSON(ctx context.Context, c *gin.Context, account *Account, body []byte) ([]byte, error) {
	if len(bytes.TrimSpace(body)) == 0 || !gjson.ValidBytes(body) {
		return body, nil
	}
	var decoded any
	...
	changed := false
	if err := s.localizeOpenAIImagesNode(ctx, c, account, decoded, &changed); err != nil {
		return nil, err
	}
	if !changed {
		return body, nil
	}
	localized, err := json.Marshal(decoded)
	if err != nil {
		return nil, err
	}
...
func (s *OpenAIGatewayService) localizeOpenAIImagesNode(ctx context.Context, c *gin.Context, account *Account, node any, changed *bool) error {
	switch value := node.(type) {
	case map[string]any:
		for key, child := range value {
			if isOpenAIImageURLField(key) {
				if rawURL, ok := child.(string); ok {
					localURL, replaced, err := s.localizeOpenAIImageURL(ctx, c, account, rawURL)
					if err != nil {
						return err
					}
					if replaced {
			...
			if err := s.localizeOpenAIImagesNode(ctx, c, account, child, changed); err != nil {
				return err
			}
		}
	case []any:
...
func (s *OpenAIGatewayService) localizeOpenAIImageURL(ctx context.Context, c *gin.Context, account *Account, rawURL string) (string, bool, error) {
	rawURL = strings.TrimSpace(rawURL)
	if rawURL == "" || isLocalImagePublicURL(rawURL) {
		return rawURL, false, nil
	}
	...
	case strings.HasPrefix(lower, "data:image/"):
		data, contentType, err = localimagestore.DecodeDataURL(rawURL)
	case strings.HasPrefix(lower, "http://") || strings.HasPrefix(lower, "https://"):
		data, contentType, err = s.downloadOpenAIImageURL(ctx, account, nil, rawURL)
	default:
		return rawURL, false, nil
	}
	if err != nil {
		return "", false, fmt.Errorf("localize image url: %w", err)
	}
	stored, err := localimagestore.Store(s.imageStorageDataDir(), data, contentType)
	if err != nil {
		return "", false, fmt.Errorf("store image locally: %w", err)
	}
	var request *http.Request
	if c != nil {
		request = c.Request
	}
	return localimagestore.PublicURL(request, s.imagePublicBaseURL(), stored.FileName), true, nil
}

func (s *OpenAIGatewayService) downloadOpenAIImageURL(ctx context.Context, account *Account, headers http.Header, downloadURL string) ([]byte, string, error) {
	if s == nil || s.httpUpstream == nil {
		errorBodyReadLimit := openAIUpstreamErrorBodyReadLimitForConfig(nil)
		if s != nil {
			errorBodyReadLimit = openAIUpstreamErrorBodyReadLimitForConfig(s.cfg)
		}
		data, err := downloadOpenAIImageBytes(ctx, req.C(), headers, downloadURL, errorBodyReadLimit)
		if err != nil {
			return nil, "", err
		}
	...
	applyOpenAIImageDownloadHeaders(downloadReq.Header, headers, downloadURL)

	proxyURL := openAIImageAccountProxyURL(account)
	accountID := int64(0)
	accountConcurrency := 0
	if account != nil {
		accountID = account.ID
	...
	resp, err := s.httpUpstream.Do(downloadReq, proxyURL, accountID, accountConcurrency)
	if err != nil {
		return nil, "", err
	}
	if resp == nil {
	...
	defer func() { _ = resp.Body.Close() }()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, "", newOpenAIImageHTTPStatusError(resp, "download image bytes failed")
	}
...
func applyOpenAIImageDownloadHeaders(dst http.Header, src http.Header, downloadURL string) {
	if dst == nil {
		return
	}
	if strings.HasPrefix(downloadURL, openAIChatGPTStartURL) {
		for key, values := range cloneHTTPHeader(src) {
			for _, value := range values {
				dst.Add(key, value)
			}
		}
		dst.Set("Accept", "image/*,*/*;q=0.8")
...
func openAIImageAccountProxyURL(account *Account) string {
...
func isLocalImagePublicURL(rawURL string) bool {
	rawURL = strings.TrimSpace(rawURL)
	if strings.HasPrefix(rawURL, localimagestore.URLPathPrefix+"/") {
		return true
	}
	...
	return strings.HasPrefix(parsed.Path, localimagestore.URLPathPrefix+"/")
}

func (s *OpenAIGatewayService) localizeOpenAIImagesStreamLine(ctx context.Context, c *gin.Context, account *Account, line []byte) ([]byte, error) {
	trimmedLine := strings.TrimRight(string(line), "\r\n")
	newline := string(line[len(trimmedLine):])
	if data, ok := extractOpenAISSEDataLine(trimmedLine); ok {
		if strings.TrimSpace(data) == "" || strings.TrimSpace(data) == "[DONE]" || !gjson.Valid(data) {
			return line, nil
		}
		localized, err := s.localizeOpenAIImagesResponseJSON(ctx, c, account, []byte(data))
		if err != nil {
			return nil, err
		}
		return []byte("data: " + string(localized) + newline), nil
	}
	if gjson.Valid(strings.TrimSpace(trimmedLine)) {
		localized, err := s.localizeOpenAIImagesResponseJSON(ctx, c, account, []byte(strings.TrimSpace(trimmedLine)))
		if err != nil {
			return nil, err
		}
		return []byte(string(localized) + newline), nil
	}
	return line, nil
}
