...
type CreateOrderResponse struct {
	...
	PayCurrencyAmount float64                    `json:"pay_currency_amount,string,omitempty"`
...
type PaymentService struct {
	...
	manualCheckMap map[int64]time.Time
}

func NewPaymentService(entClient *dbent.Client, registry *payment.Registry, loadBalancer payment.LoadBalancer, redeemService *RedeemService, subscriptionSvc *SubscriptionService, configService *PaymentConfigService, userRepo UserRepository, groupRepo GroupRepository, affiliateService *AffiliateService) *PaymentService {
	svc := &PaymentService{entClient: entClient, registry: registry, loadBalancer: newVisibleMethodLoadBalancer(loadBalancer, configService), redeemService: redeemService, subscriptionSvc: subscriptionSvc, configService: configService, userRepo: userRepo, groupRepo: groupRepo, affiliateService: affiliateService, manualCheckMap: make(map[int64]time.Time)}
...
// NowPaymentsCurrencies returns the list of available currencies from the
// NowPayments API for the registered NowPayments provider.
func (s *PaymentService) NowPaymentsCurrencies(ctx context.Context) ([]provider.NowPaymentsCurrency, error) {
	s.EnsureProviders(ctx)
	p, err := s.registry.GetProviderByKey(payment.TypeNowPayments)
	if err != nil {
		return nil, fmt.Errorf("nowpayments provider not registered: %w", err)
	}
	np, ok := p.(*provider.NowPayments)
	if !ok {
		return nil, fmt.Errorf("provider is not NowPayments")
	}
	return np.GetCurrencies(ctx)
}
