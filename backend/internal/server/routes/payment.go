...
func RegisterPaymentRoutes(
	...
	authenticated.Use(middleware.BackendModeUserGuard(settingService))
	{
		...
		orders := authenticated.Group("/orders")
		{
			...
			orders.POST("/:id/check-status", paymentHandler.CheckPaymentStatus)
	...
	webhook := v1.Group("/payment/webhook")
	{
		...
		webhook.POST("/nowpayments", webhookHandler.NowPaymentsWebhook)
	}
	...
	adminGroup.Use(middleware.AdminComplianceGuard(settingService))
	{
		...
		// NowPayments currencies (proxy)
		adminGroup.GET("/nowpayments/currencies", adminPaymentHandler.NowPaymentsCurrencies)
