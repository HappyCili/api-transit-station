...
func builtinOAuthConfigFromEnv() (OAuthConfig, error) {
	clientID := strings.TrimSpace(os.Getenv(GeminiCLIOAuthClientIDEnv))
	clientSecret := strings.TrimSpace(os.Getenv(GeminiCLIOAuthClientSecretEnv))
...
//
// If ClientID/ClientSecret is not provided, this falls back to the env-configured
// built-in Gemini CLI OAuth client.
//
...
func EffectiveOAuthConfig(cfg OAuthConfig, oauthType string) (OAuthConfig, error) {
	...
	if effective.ClientID == "" && effective.ClientSecret == "" {
		builtinCfg, err := builtinOAuthConfigFromEnv()
		if err != nil {
			return OAuthConfig{}, err
		}
		effective.ClientID = builtinCfg.ClientID
		effective.ClientSecret = builtinCfg.ClientSecret
