...
const (
	testGeminiCLIOAuthClientID     = "test-gemini-cli-client-id"
	testGeminiCLIOAuthClientSecret = "test-gemini-cli-client-secret"
)

func setBuiltinOAuthEnv(t *testing.T) {
	t.Helper()
	t.Setenv(GeminiCLIOAuthClientIDEnv, testGeminiCLIOAuthClientID)
	t.Setenv(GeminiCLIOAuthClientSecretEnv, testGeminiCLIOAuthClientSecret)
}
...
func TestBuildAuthorizationURL_UsesBuiltinSecretFallback(t *testing.T) {
	...
	if !strings.Contains(authURL, "client_id="+testGeminiCLIOAuthClientID) {
