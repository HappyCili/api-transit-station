...
func TestMain(m *testing.M) {
	ClientID = testAntigravityClientID
	os.Exit(m.Run())
}
...
func TestGetClientID_配置值(t *testing.T) {
	old := ClientID
	...
	if clientID != "my-client-id" {
		t.Errorf("应去除前后空格: got %q, want %q", clientID, "my-client-id")
	}
}

func TestGetClientID_缺失(t *testing.T) {
	old := ClientID
	ClientID = ""
	t.Cleanup(func() { ClientID = old })
