    ...
    <template v-else-if="qrUrl">
      <div class="card p-6">
        <div class="flex flex-col items-center space-y-4">
          ...
          <!-- NowPayments crypto address -->
          <div v-if="isNowPayments" class="w-full space-y-2">
            <div class="rounded-lg bg-gray-50 p-3 dark:bg-dark-700">
              <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.nowpayments.sendAmount') }}</p>
              <p class="text-lg font-bold text-gray-900 dark:text-white">{{ payAmountDisplay }}</p>
            </div>
            <div class="rounded-lg bg-gray-50 p-3 dark:bg-dark-700">
              <p class="text-xs text-gray-500 dark:text-gray-400">{{ t('payment.nowpayments.walletAddress') }}</p>
              <div class="mt-1 flex items-start gap-2">
                <p class="flex-1 break-all text-sm font-mono text-gray-900 dark:text-white select-all leading-relaxed">{{ props.qrCode }}</p>
                <button class="shrink-0 rounded-md p-1.5 text-gray-400 hover:bg-gray-200 hover:text-gray-600 dark:hover:bg-dark-600 dark:hover:text-gray-300 transition-colors" :title="t('common.copyToClipboard')" @click="copyToClipboard(props.qrCode, t('payment.nowpayments.addressCopied'))">
      ...
      <div class="flex gap-2">
        <button class="btn btn-secondary flex-1" :disabled="checkingStatus" @click="handleManualCheck">
          <span v-if="checkingStatus" class="inline-block h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent mr-1"></span>
          {{ checkingStatus ? t('common.processing') : t('payment.qr.checkStatus') }}
        </button>
        <button class="btn btn-secondary flex-1" :disabled="cancelling" @click="handleCancel">
          {{ cancelling ? t('common.processing') : t('payment.qr.cancelOrder') }}
        </button>
      </div>
      <p v-if="statusCheckError" class="text-xs text-red-500 text-center">{{ statusCheckError }}</p>
    </template>
...
const payAmountDisplay = computed(() => {
  if (isNowPayments.value && props.payCurrencyAmount != null && props.payCurrencyAmount > 0) {
    const cur = props.payCurrency?.toUpperCase() || ''
    return `${props.payCurrencyAmount} ${cur}`
  }
  if (props.payAmount != null && props.payAmount > 0) {
    const cur = props.currency?.toUpperCase() || ''
    return `${props.payAmount} ${cur}`
  }
  return ''
})
