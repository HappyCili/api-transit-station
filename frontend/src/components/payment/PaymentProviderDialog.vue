...
const resolvedFields = computed(() => {
  ...
  return fields.map(f => {
    ...
    if (form.provider_key === 'nowpayments' && f.key === 'payCurrency' && payCurrencyOptions.value.length > 0) {
      resolved.options = payCurrencyOptions.value
    }
    return resolved
  })
})
...
function reset(defaultKey: string) {
  ...
  clearConfig()
  applyDefaults()
  if (defaultKey === 'nowpayments') fetchPayCurrencyOptions()
}
...
function loadProvider(provider: ProviderInstance) {
  ...
  applyDefaults()
  if (provider.provider_key === 'nowpayments') fetchPayCurrencyOptions()
